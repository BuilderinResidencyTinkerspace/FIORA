...
Our model was inspired by Bella, a delivery robot, so we took a similar design as our reference. We planned to start by building the base using a foam sheet. We cut the sheet into a circular shape with a radius of ___ cm to form the base of the model and took measurements of the mecanum wheels. We then cut the sheet according to the wheel measurements, completing the basic base structure of the prototype.

Next, we needed to design a suitable mounting model to fit and secure the motors to the base. At the same time, we realised that our motor-to-wheel hub adapter could not directly fit into the wheel hub. We got stuck at this stage, so we searched online for a suitable adapter. However, the available options were too expensive, so we decided to make one ourselves.
